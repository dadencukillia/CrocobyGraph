...
namespace CrocobyGraph {
  ...
  void Physics::init(bool jelly, Scene* scene, GraphECS* ecs) {
    ...
    if (jelly) {
      const float angle_segment { 2.0f * PI / jelly_points };
      for (size_t i = 0; i < jelly_points; ++i) {
        const float angle { static_cast<float>(i) * angle_segment };
  ...
  void Physics::update_velocity(double delta) {
    ...
    for (const auto& [edge_entity, edge] : registry.view<const EdgeEntity>().each()) {
      ...
      const Vector2 vector { a_pos.x - b_pos.x, a_pos.y - b_pos.y };
      const float distance_square { vector.x * vector.x + vector.y * vector.y + 0.1f };
      const float distance { std::sqrt(distance_square) };
      const float force { PHYSICS_ATTRACTION_CONSTANT * (distance - PHYSICS_ATTRACTION_IDEAL_DISTANCE) };

      const float force_apply_denominator { 1.0f / distance * force };

      const Vector2 force_apply {
        vector.x * force_apply_denominator,
        vector.y * force_apply_denominator
      };

      const float vel_denominator { 1.0f / (b_node.radius * b_node.radius) };
    ...
    const auto thread_worker = [delta, &registry](const entt::entity* start, const size_t len) {
      for (size_t i = 0; i < len; ++i) {
        ...
        const float mass { node.radius * node.radius };
        ...
        for (const auto& [another_entity, another_repulsion, another_pos] : registry.view<const RepulsionComponent, const PositionComponent>().each()) {
          ...
          const Vector2 vector { pos.x - another_pos.x, pos.y - another_pos.y };
          const float distance_square { vector.x * vector.x + vector.y * vector.y + 0.1f };
          const float distance { std::sqrt(distance_square) };
          const float force { PHYSICS_REPULSION_CONSTANT * another_repulsion.charge * repulsion.charge / (distance_square * distance) };
          forces.x += vector.x * force;
          forces.y += vector.y * force;
        }

        // Gravity force
        const float center_distance { std::sqrt(pos.x * pos.x + pos.y * pos.y) + 0.1f };
        const float inv_center_distance { 1.0f / center_distance };
        const Vector2 gravity_direction { -pos.x * inv_center_distance, -pos.y * inv_center_distance };
        ...
        const Vector2 acceleration { forces.x * inv_mass, forces.y * inv_mass };
    ...
    if (velocity_count >= 128 && PHYSICS_CALCULATIONS_THREADS != 0) {
      ...
      std::array<size_t, PHYSICS_CALCULATIONS_THREADS> task_ids;
  ...
  void Physics::update_jelly(double delta) {
    ...
    for (const auto& [entity, node, pos, velocity, jelly] : registry.view<const NodeEntity, const PositionComponent, const VelocityComponent, JellyComponent>().each()) {
      const float vel_value { std::sqrt(velocity.x * velocity.x + velocity.y * velocity.y) + 0.01f };
      const Vector2 vel_dir { velocity.x / vel_value, velocity.y / vel_value };
      bool is_nearly_ideal { true };

      for (size_t i = 0; i < jelly_points; ++i) {
        const auto point_dir { jelly_ideal_points[i] };
        const float diff { vel_dir.x * point_dir.x + vel_dir.y * point_dir.y };
        const float impact_mul { (diff + 1.0f) / 2.0f };
        const float total_mul { static_cast<float>((impact_mul * impact_mul + 0.05f) * delta * TARGET_TPS) };
        const Vector2 ideal_pos { 
          pos.x + point_dir.x * node.radius,
          pos.y + point_dir.y * node.radius
        };
        const float squish_factor { vel_value * 0.02f };
        const Vector2 deformed_ideal {
        ...
        const float dist_to_ideal { std::abs(jelly_point.x - ideal_pos.x) + std::abs(jelly_point.y - ideal_pos.y) };
