...
namespace CrocobyGraph {
  ...
  Vector2 get_label_position(const entt::registry& registry, entt::entity entity) {
    if (registry.any_of<AttachComponent>(entity)) {
      return get_attach_label_position(registry, entity);
    } else {
      return get_free_label_position(registry, entity);
    }
  }
  ...
  Vector2 get_free_label_position(const entt::registry& registry, entt::entity entity) {
    const auto& pos_component = registry.get<const PositionComponent>(entity);
    return { pos_component.x, pos_component.y };
  }

  Vector2 get_attach_label_position(const entt::registry& registry, entt::entity entity) {
