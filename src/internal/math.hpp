...
namespace CrocobyGraph {
  ...
  [[nodiscard]] Vector2 calculate_bezier_cubic_dot(
    Vector2 start, Vector2 end,
    Vector2 c1, Vector2 c2,
    float divisions, float index
  ) noexcept;

  [[nodiscard]] Vector2 calculate_bezier_cubic_in_out_dot(
    Vector2 a, Vector2 b,
    float segments, float index
  ) noexcept;

  [[nodiscard]] ApproximatelyIntersectResult approximately_circle_intersection(
  ...
  [[nodiscard]] bool check_point_in_rect(
    Vector2 point,
    Vector2 rect_top_left, Vector2 rect_bottom_right
  ) noexcept;

  [[nodiscard]] bool check_rect_collision_line(
    Vector2 line_start, Vector2 line_end,
    Vector2 rect_top_left, Vector2 rect_bottom_right
  ) noexcept;

  [[nodiscard]] bool check_rect_a_in_rect_b(
    Vector2 rect_a_top_left, Vector2 rect_a_bottom_right,
    Vector2 rect_b_top_left, Vector2 rect_b_bottom_right
  ) noexcept;

  [[nodiscard]] bool approximately_check_bezier_line_in_rect(
    std::function<Vector2(ApproximatelySplineCallbackParams)> spline_dot_function,
    Vector2 rect_top_left, Vector2 rect_bottom_right,
    float area_threshold = 2.0f
  );

  [[nodiscard]] inline Vector2 translate_world_to_screen_coordinates(
    Vector2 screen_pos, 
    Vector2 camera_pos, float camera_zoom,
    Vector2 window_size
  ) noexcept {
  ...
  [[nodiscard]] constexpr Vector2 normalize_vector(Vector2 vec) noexcept {
    const float len_sqr { vec.x * vec.x + vec.y * vec.y };
